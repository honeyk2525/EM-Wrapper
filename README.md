...
A Windows desktop GUI that wraps the GROMACS command-line pipeline for **energy minimization of protein structures**. Select one or more `.pdb` files, pick a force field, and let it run — no terminal required.

---

## Features

- ✅ **Multi-file batch runs** — Process up to 25+ files
- ✅ **Per-job status table** — File · Status · Step columns
- ✅ **Live log viewer** — Click any job row to view logs
- ✅ **Cancel All** — Stop all running jobs
- ✅ **Open output folder** — Per selected job
- ✅ **Final PDB collection** — Auto-copies all results to `minimized_pdbs/`
- ✅ **Force field selector** — 14 fields available
- ✅ **Settings dialog** — Custom `gmx` binary path
- ✅ **Persistent config** — `~/.gmx_runner_config.json`
...
- **Python 3.8+** with `tkinter` (included with standard Python installation)
...
- Example: `C:\Program Files\GROMACS\bin\gmx.exe`
